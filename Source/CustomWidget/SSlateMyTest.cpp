...
void SSlateMyTest::Construct(const FArguments& InArgs)
{
...
int32 SSlateMyTest::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect,
	...
	bool bParentEnabled) const
{
	...
	const FSlateClippingZone ClippingZone(
		FVector2D(Center.X - Size.X * 0.25, Center.Y - Size.Y * 0.25),
		FVector2D(Center.X + Size.X * 0.25, Center.Y - Size.Y * 0.25),
		FVector2D(Center.X - Size.X * 0.25, Center.Y + Size.Y * 0.25), 
		FVector2D(Center.X + Size.X * 0.25, Center.Y + Size.Y * 0.25));
...
void SSlateMyTest::SetBrush(FSlateBrush* InBrush)
{
	Brush.SetImage(*this, InBrush);
}
void SSlateMyTest::SetImage(TAttribute<const FSlateBrush*> InImage)
{
	Image.SetImage(*this, InImage);
}
void SSlateMyTest::TestSpline(const FGeometry& InParams)
{

}


bool SSlateMyTest::SupportsKeyboardFocus() const
{
	return true;
}

FReply SSlateMyTest::OnMouseButtonUp(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SSlateMyTest::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	FReply Reply = FReply::Unhandled();
...
void SSlateMyTest::OnMouseLeave(const FPointerEvent& MouseEvent)
{
	bIsHovered = false;
...
void SSlateMyTest::OnMouseEnter(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	bIsHovered = true;
