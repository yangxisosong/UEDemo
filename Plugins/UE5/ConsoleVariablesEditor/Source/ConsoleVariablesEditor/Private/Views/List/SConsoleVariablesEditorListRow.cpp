...
FText SConsoleVariablesEditorListRow::MultiDragFormatText = LOCTEXT("MultiDragFormatText", "{0} Items");
...
TSharedRef<SWidget> SConsoleVariablesEditorListRow::GenerateWidgetForColumn(const FName& InColumnName)
{
	...
	const TSharedRef<SImage> FlashImage = SNew(SImage)
										.Image(new FSlateColorBrush(FLinearColor(0.2f,.2f,.2f,1.f)));
										//.Visibility_Raw(
	...
	return SNew(SOverlay)
			...
			[
				SNew(SBorder)
					...
					.BorderBackgroundColor(FLinearColor(0.2f,.2f,.2f,1.f))
...
void SConsoleVariablesEditorListRow::OnCheckboxStateChange(const ECheckBoxState InNewState) const
{
	if (const TSharedPtr<FConsoleVariablesEditorListRow> PinnedItem = Item.Pin())
	{
		...
		const TWeakPtr<SConsoleVariablesEditorList> ListView = PinnedItem->GetListViewPtr();
		if ( ListView.IsValid())
		{
	...
	const TSharedPtr<FConsoleVariablesEditorListRow> PinnedItem)
{
	if (PinnedItem->GetCommandInfo().IsValid())
	{
		...
		const TSharedRef<SHorizontalBox> FinalValueWidget =
			...
			{const TSharedPtr<FConsoleVariablesEditorCommandInfo> PinnedCommand = PinnedItem->GetCommandInfo().
				Pin();
				if (
                                                           ...
                                                           const TWeakPtr<FConsoleVariablesEditorListRow> InRow)
{
	...
	const FSlateBrush* ButtonImage = new FSlateColorBrush(FLinearColor::Red); //FAppStyle::Get().GetBrush("Icons.Delete");
