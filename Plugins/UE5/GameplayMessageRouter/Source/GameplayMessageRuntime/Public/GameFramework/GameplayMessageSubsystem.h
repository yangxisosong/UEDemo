...
public:
		...
		EGameplayMessageMatch MatchType)
	{
		...
		auto ThunkCallback = [InnerCallback = Callback](FGameplayTag ActualTag, const UScriptStruct* SenderStructType, const void* SenderPayload)
		{
			InnerCallback(ActualTag, SenderStructType, SenderPayload);
		};

		Handle = RegisterListenerInternal(Channel, ThunkCallback, StructType, MatchType);
